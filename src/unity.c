...
{
  if ((style & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT) { UnityPrintNumber(number); }
...
void UnityPrintNumberHex(const UNITY_UINT number, const char nibbles_to_print)
{
  ...
  while (nibbles > 0)
  {
    ...
    if (nibble <= 9) { UNITY_OUTPUT_CHAR((char)('0' + nibble)); }
    else { UNITY_OUTPUT_CHAR((char)('A' - 10 + nibble)); }
  }
}
...
void UnityPrintMask(const UNITY_UINT mask, const UNITY_UINT number)
{
  ...
  for (i = 0; i < UNITY_INT_WIDTH; i++)
  {
    if (current_bit & mask)
    {
      if (current_bit & number) { UNITY_OUTPUT_CHAR('1'); }
      else { UNITY_OUTPUT_CHAR('0'); }
    }
    else { UNITY_OUTPUT_CHAR('X'); }
...
static void UnityPrintDecimalAndNumberWithLeadingZeros(UNITY_INT32 fraction_part,
                                                       UNITY_INT32 divisor)
{
  UNITY_OUTPUT_CHAR('.');
  while (divisor > 0)
                              ...
                              const UNITY_LINE_TYPE lineNumber, const UNITY_DISPLAY_STYLE_T style)
{
  ...
  {
    if (actual > expected) { Unity.CurrentTestFailed = ((UNITY_UINT)(actual - expected) > delta); }
    else { Unity.CurrentTestFailed = ((UNITY_UINT)(expected - actual) > delta); }
  }
  else
  {
                                 ...
                                 const UNITY_LINE_TYPE lineNumber)
{
  ...
  do {
    ...
    { /* handle case of one pointers being null (if both null, test should pass) */
      if (expected[j] != actual[j]) { Unity.CurrentTestFailed = 1; }
    }
                            ...
                            const char* msg, const UNITY_LINE_TYPE lineNumber)
{
  ...
  if ((elements == 0) || (length == 0)) { UnityPrintPointlessAndBail(); }
...
int UnityEnd(void)
{
  ...
  UNITY_PRINT_EOL();
  if (Unity.TestFailures == 0U) { UnityPrint(UnityStrOk); }
  else
  {
    UnityPrint(UnityStrFail);
    ...
    if (argv[i][0] == '-')
    {
      switch (argv[i][1])
      {
        ...
        case 'f': /* an alias for -n */
          if (argv[i][2] == '=') { UnityOptionIncludeNamed = &argv[i][3]; }
          else if (++i < argc) { UnityOptionIncludeNamed = argv[i]; }
          else
          {
        ...
        case 'x': /* exclude tests with name including this string */
          if (argv[i][2] == '=') { UnityOptionExcludeNamed = &argv[i][3]; }
          else if (++i < argc) { UnityOptionExcludeNamed = argv[i]; }
          else
          {
...
int UnityTestMatches(void)
{
  ...
  int retval;
  if (UnityOptionIncludeNamed) { retval = UnityStringArgumentMatches(UnityOptionIncludeNamed); }
  else { retval = 1; }
